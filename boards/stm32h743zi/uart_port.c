/**
 * @file    uart_port.c
 * @brief   STM32H743ZI UART 硬件实现
 * @note    使用 USART3 (PB10=TX, PB11=RX) + DMA1_Stream1
 *
 * 实现说明：
 * - DMA 循环接收，零拷贝设计
 * - 空闲中断触发数据处理
...
#define UART_INSTANCE        USART3  /* 使用 USART3 */
...
static bool hal_uart_init(uint32_t baudrate)
{
    ...
    /* 配置 DMA: DMA1_Stream1 用于 USART3_RX */
